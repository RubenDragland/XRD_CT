function [finish] = Run_SH(parent, sample_name, mode, filter, varargin)

% Many params in this case. 
% parent, sample_name, filter, mode,
%a0 = 1, theta_init = pi/4, phi_init = pi/4
%it1 = 10, it2 = 20, 
%it3 = 20, it4 = 50, 
default_a = 1e-4;
default_sign = 1;
...
p.mode = mode;                      % RSD: 1 for AD, 0 for symbolic
p.method = "bilinear";          % RSD: Choose method of interpolation.
p.filter_2D = filter; %1;                % RSD: The best filter.
p.GPU = 0;
p.python = mode; %RSD: Improve safety here.
p.batch = mode; %RSD: ALWAYS
p.plot = 0;
...
p.itmax = it1; %20                % maximum number of iterations: about 20
...
p.itmax = it2; %50; %30;           % maximum number of iterations: about 50
p.skip_projections = 1; % = 1, for not skipping projections
...
p.itmax = it3; %20;            % maximum number of iterations: about 20
p.skip_projections = 1;  % = 1, for not skipping projections
...
p.itmax = it4; %50;                          % maximum number of iterations: about 50-100
p.skip_projections = 1;                % = 1, for not skipping projections
