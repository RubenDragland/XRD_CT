...
% RSD: This procedure skips the arb_back_projection. This may not be so clever, since the function does a transformation from 2D images to 3D array
...
if find_grad
    a_temp = dlarray(a_temp); % RSD: Also fix function call
...
function [error_norm, ad_grad, aux_diff_poisson, proj_out_all ] = SAXS_AD_cost_function(a_temp, Ylm, ny, nx, nz, numOfsegments, data, current_projection, Rot_exp_now, p, find_grad, X, Y, Z, numOfpixels)
