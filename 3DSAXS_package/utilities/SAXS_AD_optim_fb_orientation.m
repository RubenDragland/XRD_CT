...
function [E, AD_grad_coeff, AD_grad_theta, AD_grad_phi] = SAXS_AD_optim_fb_orientation(theta_struct, phi_struct, a_temp, ny, nx, nz, numOfsegments, projection, p, find_grad, X, Y, Z, numOfpixels, unit_q_beamline, Ylm_coef, find_coefficients, numOfCoeffs, numOfvoxels)
    ...
    for ii = 1:length(projection) % RSD: Debug for loop  
        ...
        q_pp = pagemtimes(Rot_str, unit_q_object);
        ...
        block_cos_theta_powers =  dlarray_repmat_cumprod_SH(ones_struct, cos_theta_sh_cut, numOfCoeffs);
        ...
        sumlm_alm_Ylm = pagemtimes(a_temp, Ylm); %RSD: May be an issue here for optimise all. Allowed when unformatted
    ...
    AD_grad_theta = dlgradient(E, theta_struct);
    AD_grad_phi = dlgradient(E, phi_struct);
