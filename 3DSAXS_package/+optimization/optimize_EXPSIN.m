...
opt_inputs = [s.theta.data(:); s.phi.data(:); s.A.data(:); s.B.data(:)];
...
opt_out = optimization.cgmin1('optimization.SAXS_EXPSIN_3D_err_AD', opt_inputs, itmax, ftol, xtol, p, s, opt_projection);
...
s.A.data = reshape(opt_out(1:numOfvoxels),Nvol);
s.B.data = reshape(opt_out(numOfvoxels+1:2*numOfvoxels),Nvol);


% Save relevant data in the output file
if p.save.output_filename
    e = Err_hist;
    save(p.save.output_filename, 's', 'p', 'e', 'timing', 'tot_time', 'Err_hist', '-v6'); 
end
