...
parfor ii = 1:length(projection) %use parallel processing for the loop over all projections
    ...
    q_pp = bsxpagemult(double(Rot_str), double(unit_q_object));
    ...
    if all(mod(l,2) == 0) && all(m == 0) % Make sure that all l is even, and that all m = 0
        block_cos_theta_powers = double([ones_struct; cumprod(repmat(cos_theta_sh_cut.^2, numOfCoeffs-1, 1), 1)]);
    ...
    Ylm = bsxpagemult(double(Ylm_coef), block_cos_theta_powers);
    
    % sum_lm(a_lm*Y_lm)
    %   [1 numOfsegments numOfvoxels] = [1 numOforders numOfvoxels] * [numOforder numOfsegments numOfvoxels]
    sumlm_alm_Ylm = bsxpagemult(double(a_temp), Ylm);
    ...
    xout = (1:size(data,2)) - ceil(size(data,2)/2) + projection(ii).dx;
    yout = (1:size(data,1)) - ceil(size(data,1)/2) + projection(ii).dy;
    
    % I [y x numOfsegments] Eq(3) of Liebi et al
    [proj_out_all, xout, yout] = arb_projection(data_synt_vol, X, Y, Z, Rot_exp_now, p, xout, yout);
