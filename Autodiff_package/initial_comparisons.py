...
def symbolic_h(x1, x2) -> np.array:
    return (
        ...
        * ((1 - 2 * x1) ** 2)
        * (1 - 8 * x1 + 8 * x1**2)
        + 64 * (1 - x1) * ((1 - 2 * x1) ** 2) * ((1 - 8 * x1 + 8 * x1**2) ** 2)
        - (64 * x1 * (1 - 2 * x1) ** 2) * (1 - 8 * x1 + 8 * x1**2) ** 2
        - 256 * x1 * (1 - x1) * (1 - 2 * x1) * (1 - 8 * x1 + 8 * x1**2) ** 2
    )
...
def AD_torch_general(func, *args, **kwargs) -> torch.tensor:
    ...
    **kwargs: dict
    ...
    x = torch.ones(
        size=(len(kwargs), *kwargs[first_var].size())
    )  # , requires_grad=True)
...
if __name__ == "__main__":
    ...
    grad = AD_torch(
        interpolation_test,
        np.arange(1, 10, 1, dtype=np.float32),
        np.arange(1, 10, 1, dtype=np.float32),
    )
    ...
    times_dict = {"Symbolic": [], "AD Torch": [], "AD Autograd": [], "Numerical": []}

    grad_dict = {"Symbolic": [], "AD Torch": [], "AD Autograd": [], "Numerical": []}
